...
export class AppService {
  ...
  async emitKrakenData(data: string, name: string) {
    try {
        const responseData = await this.makeResponseData(data, name);
        this.webSocketService.broadcastMessage(responseData);
    } catch (error) {
        console.error(`Error emitting Kraken data: ${error.message}`);
        // Handle error if makeResponseData fails
    }
  }
}
